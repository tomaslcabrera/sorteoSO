...
# ==================================================================
# ===== COMIENZO: TODO EL CÓDIGO ORIGINAL DE so.py ESTÁ AQUÍ ======
# ==================================================================
...
# ================== Orquestación ==================
def main_simulacion():
    # Renombrada de 'main' a 'main_simulacion' para evitar confusión
...
# ================================================================
# ===== FIN: CÓDIGO ORIGINAL DE so.py ======
# ================================================================


# ================================================================
# ===== COMIENZO: CÓDIGO DE LA GUI (TKINTER) ======
# ================================================================

class QueueRedirector:
    """
    Un objeto tipo 'file' que redirige 'write()' (usado por 'print')
    a una 'queue.Queue' de forma segura entre hilos.
    """
    ...
    def flush(self):
        # Requerido por la interfaz de 'file', pero no necesitamos hacer nada.
        pass

class App:
    def __init__(self, root):
        self.root = root
        self.root.title("Simulación de Sorteo con Threads")
        self.root.geometry("700x500")

        self.text_area = scrolledtext.ScrolledText(root, wrap=tk.WORD, state=tk.DISABLED)
        ...
        # Creamos la cola para comunicar los hilos de simulación con la GUI
        self.gui_queue = queue.Queue()

        # Iniciamos el "poller" que revisa la cola
        self.check_gui_queue()

    def check_gui_queue(self):
        """
        Revisa la cola en busca de nuevos mensajes de 'print' y los
        añade al widget de texto. Se auto-agenda para ejecutarse
        cada 100ms en el hilo de la GUI.
        """
        try:
            while True:
                # Obtenemos un mensaje de la cola sin bloquear
                msg = self.gui_queue.get_nowait()
                
                # Habilitamos el widget, insertamos texto y deshabilitamos
            ...
            # Volvemos a llamar a esta función en 100ms
            self.root.after(100, self.check_gui_queue)

    def simulation_task_wrapper(self):
        """
        Esta función se ejecuta en el HILO SECUNDARIO.
        Ejecuta la simulación y se asegura de restaurar el
        stdout y reactivar el botón cuando termina.
        """
        try:
            # Redirigimos 'print' (stdout) a nuestra cola
            sys.stdout = QueueRedirector(self.gui_queue)
            
            # Ejecutamos la función main original
            main_simulacion()
            
        except Exception as e:
            # Imprimimos cualquier error a la GUI
            print(f"\n¡ERROR EN LA SIMULACIÓN!: {e}\n")
        finally:
            # Restauramos el stdout original
            sys.stdout = self.original_stdout
            # Reactivamos el botón (programándolo en el hilo de la GUI)
    ...
    def start_simulation(self):
        ...
        # Iniciamos la simulación en un hilo separado
        # 'daemon=True' asegura que el hilo muera si cerramos la ventana
        threading.Thread(target=self.simulation_task_wrapper, daemon=True).start()
...
if __name__ == "__main__":
    ...
    # Iniciamos el bucle de eventos de la GUI
    root.mainloop()

    # Si la ventana se cierra, restauramos stdout por si acaso
